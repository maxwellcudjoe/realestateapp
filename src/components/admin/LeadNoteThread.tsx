  ...
  return (
    <div className="space-y-3">
      <form onSubmit={add} className="space-y-2">
        <textarea value={text} onChange={(e) => setText(e.target.value)} rows={3} className="w-full border border-stone-300 rounded px-3 py-2" placeholder="Add a note about this conversation…" />
        {error && <p className="text-rose-700 text-xs">{error}</p>}
        <button type="submit" disabled={pending || !text.trim()} className="bg-stone-900 text-white text-sm px-3 py-1 rounded disabled:opacity-50">
          {pending ? 'Adding…' : 'Add note'}
        </button>
      </form>
      <ul className="divide-y divide-stone-200 border border-stone-200 rounded">
        {notes.length === 0 && <li className="px-3 py-2 text-sm text-stone-500">No notes yet.</li>}
        {notes.map((n) => (
          <li key={n.id} className="px-3 py-2 text-sm">
            <div className="text-xs text-stone-500">{new Date(n.createdAt).toLocaleString('en-GB')}</div>
